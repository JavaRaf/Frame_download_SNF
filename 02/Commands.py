...
class FinderCommands:
    ...
    def get_gif_props(self, person):
        ...
        post_infos = re.findall(
            r"\d+", self.facebook.get_post_infos(person.get("post_id"))
        )
        if post_infos:
            ...
            raise ValueError(
                "Não foi possível encontrar as informações do GIF nos dados fornecidos."
            )
    ...
    def add_help_message(self, person):
        ...
        person["help_filepath"] = os.path.join(
            os.path.dirname(__file__), "images", "help.png"
        )
        return person
    ...
    def get_post_infos(self, person):
        try:
            ...
            if (
                "season" not in person
                or "episode" not in person
                or "frame" not in person
            ):
                post_infos = self.facebook.get_post_infos(person.get("post_id"))
                if post_infos:
                    match = re.match(
                        r"Season (\d+), Episode (\d+), Frame (\d+) out of (\d+)",
                        post_infos,
                    )
                    if match:
