...
class Facebook:
    ...
    def fetch_post_info(self, person):
        ...
        try:
            ...
            post_info = re.match(r"Season (\d+), Episode (\d+), Frame (\d+)", data['message'])
            if post_info:
    ...
    def fetch_page_name_and_id(self) -> tuple:
        ...
        try:
            ...
            if 'id' not in data or 'name' not in data:
                raise ValueError("Response JSON is missing required fields: 'id' or 'name'")
    ...
    def process_comments(self, comments):
        ...
        for comment in comments:
            ...
            for data_comment in data_comments:
                if (data_comment["id"] not in self.replied_ids) and data_comment["message"].startswith("!"):
                    person = {
                        ...
                        "post_id": self.page_id + "_" + data_comment.get("id").split("_")[0],
                    }
    ...
    def search_data(self):
        ...
        max_retries = 1
        data = []
        params = {"fields": "comments.limit(100)", "limit": "100", "access_token": self.token}

        while tries < max_retries:
            try:
                response = httpx.get(f"{self.base_url}/me/posts", params=params, timeout=15)
                ...
                if response.status_code == 200:
                    ...
                    if response_data:
                        ...
                        after = response_data.get("paging", {}).get("cursors", {}).get("after", None)
                        if after:
