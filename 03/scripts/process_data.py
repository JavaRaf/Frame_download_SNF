...
class ProcessData:
    ...
    def add_subtitle(person):
        ...
        font_path = os.path.join(
            os.path.abspath(
                os.path.join(os.path.dirname(__file__), "..", "font", "Cooper.otf")
            ).replace("\\", "/")
        )
        font_size = "-pointsize"
        font_size_value = "100"
        ...
        )  #  magick to windows || convert to linux
    ...
    def add_help_message(person):
        ...
        person["file_path"] = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "images", "help.png")
        )
