...
def get_one_img(comment: dict) -> None:
    response = httpx.get(
        f'{data.github_url}/frame_{comment["frame_number"]}.jpg', timeout=20
    )
    ...
    if response.status_code == 200:
        ...
        print(
            f"Error downloading frame {comment['frame_number']}: {response.status_code} {response.text}"
        )
...
async def get_manys_img(session: httpx.AsyncClient, frame_number: str, id: str) -> None:
    try:
        response = await session.get(
            f"{data.github_url}/frame_{frame_number}.jpg", timeout=15
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error downloading frame {frame_number}: {response.status_code} {response.text}"
            )
            return ""
    ...
    if user.str_command_download in comment.get("comment", "").lower():
        ...
        async with httpx.AsyncClient() as session:
            ...
            if file_paths[0] != "":
                ...
                save_ids.save_id(
                    comment, "Imagem(s) não encontrada(s), reposta ignorada, id salvo: "
                )
