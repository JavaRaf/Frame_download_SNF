    ...
    while retries < 3:
        try:
            ...
            with open(comment["file_path"], "rb") as frame:
                files = {
                    "file": (os.path.basename(comment["file_path"]), frame, mimetype)
                }
    ...
    while retries < 3:
        ...
        if comment.get("file_path") and comment["file_path"].endswith(".jpg"):
            ...
            if comment.get("message") and comment["message"].startswith("Helper"):
                message = (
                    comment["message"]
                    + f'{user.message_response_helper.format(LINK_GIF=comment["link"])}'
                )
            else:
        ...
        dados = {"message": message, "access_token": data.FB_TOKEN}
        if comment.get("foto_id"):
            dados["attachment_id"] = comment["foto_id"]

        response = httpx.post(
            f'{data.fb_url}/{comment["id"]}/comments', data=dados, timeout=20
        )
