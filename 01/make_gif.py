        ...
        try:
            async with httpx.AsyncClient() as client:
                with open(comment["file_path"], "rb") as gif_file:
                    files = {"file": gif_file}
                    dados = {"api_key": data.GIPHY_API_KEY}
                    response = await client.post(
                        data.giphy_url, files=files, params=dados, timeout=30
                    )
                    if response.status_code == 200:
                        response_data = response.json()
                        link = "https://giphy.com/gifs/" + response_data.get(
                            "data", {}
                        ).get("id", "")
                        comment["link"] = link
                        ...
                        print(
                            f"Erro ao enviar a GIF: {response.status_code}, {response.text}"
                        )
...
async def make_gif(comment: dict) -> None:
    ...
    if data.GIPHY_API_KEY and comment.get("file_path", None):
        ...
        if file_path:
            ...
            resize_image_command_base = (
                ["magick", "mogrify"] if os.name == "nt" else ["mogrify"]
            )
